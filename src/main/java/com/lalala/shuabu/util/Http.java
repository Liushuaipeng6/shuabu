...
public class Http {
    ...
    public String mainHandler(User user) {
        HashMap<String, String> map = new HashMap();
        map.put(user.getUser(), user.getPassword());
        int stepNum = user.getStep();
        ...
        while(var5.hasNext()) {
            ...
            try {
                ...
                String accessCode = this.getAccessCode(string, s);
                Map<String, String> login = this.login(accessCode);
                String login_token = (String)login.get("login_token");
                String user_id = (String)login.get("user_id");
                String appToken = this.getAppToken(login_token);
                String time = this.getTime();
                return this.updateStep(appToken, user_id, time, stepNum);
    ...
    public String getAccessCode(String account, String password) {
        try {
            ...
            httpPost.setConfig(RequestConfig.custom().setSocketTimeout(5000).setRedirectsEnabled(false).build());
    ...
    public String updateStep(String appToken, String userId, String time, Integer step) {
        Date date = new Date();
